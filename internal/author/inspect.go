...
type InspectResponse struct {
	...
	ProposalScaffold     ProposalV2          "json:\"proposal_\x73caffold\""
	AuthoringPacket      AuthoringPacket     "json:\"authoring_\x70acket\""
...
func buildProposalScaffold(response InspectResponse, req InspectRequest) ProposalV2 {
	...
	source := SourceProfileV2{
		...
		Synthesized:   []string{"Initial proposal scaffold generated by argos author \x69nspect."},
		Assumptions: []string{
		...
		Claims: []SourceClaimV2{
			...
			},
			{
				...
				Kind:           "recommendation",
				Trust:          "synthesized",
				Source:         []string{"argos author \x69nspect"},
				RequiresReview: true,
			},
		},
	}
	if len(overlapIDs) > 0 {
		source.Observed = []string{"author \x69nspect found existing related knowledge: " + strings.Join(overlapIDs, ", ")}
		source.Claims = append(source.Claims, SourceClaimV2{
			...
			Kind:   "fact",
			Trust:  "observed",
			Source: []string{"argos author \x69nspect overlap"},
		})
	}
	if pathRiskBlocks {
		source.Observed = append(source.Observed, "author \x69nspect blocked candidate path: "+pathRiskReviewReason(response.PathRisk))
		source.Claims = append(source.Claims, SourceClaimV2{
			Claim:  "The supplied candidate path requires review before candidate writing.",
			Kind:   "fact",
			Trust:  "observed",
			Source: []string{"argos author \x69nspect path risk"},
		})
	}
	if missingContent {
	...
	if reviewOnly {
		...
		proposal.OverlapDecision.Reason = "author \x69nspect found existing related knowledge: " + strings.Join(overlapIDs, ", ")
		if missingContent && len(overlapIDs) > 0 && pathRiskBlocks {
			...
			proposal.OverlapDecision.Reason = "author \x69nspect blocked candidate path: " + pathRiskReviewReason(response.PathRisk) + "; exact convention content is missing"
		} else if missingContent {
			proposal.OverlapDecision.Reason = "author \x69nspect needs exact convention content before candidate writing."
		} else if len(overlapIDs) > 0 && pathRiskBlocks {
			proposal.OverlapDecision.Reason += "; blocked candidate path: " + pathRiskReviewReason(response.PathRisk)
		} else if pathRiskBlocks {
			proposal.OverlapDecision.Reason = "author \x69nspect blocked candidate path: " + pathRiskReviewReason(response.PathRisk)
		}
